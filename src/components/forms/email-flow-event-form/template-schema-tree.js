...
const TemplateSchemaTree = ({ template_schema }) => {
    ...
    const formatLabel = (name, type) => type ? `${name} (${type})` : name;
    ...
    const expand = (name, def) => {
        ...
        if (def.type === 'array') {
            ...
            props.forEach((prop, index) => {
                const def = prop[1];
                if (def.type === 'object') {
                    const exp = expand(prop[0], {type: def.type, properties: def.properties});
                    const child = res.children[index];
                    if (child) child.children = exp.children;
                }
            });
